...
describe('WorkerTableComponent', () => {
  ...
  it('should have green header color', () => {
    ...
    expect(color).toBe('worker-table--up');
  });

  it('should return correct class for each state', () => {
    const componentRef = fixture.componentInstance;

    expect(componentRef.getStateColor('UP')).toBe('worker-table--up');
    expect(componentRef.getStateColor('DOWN')).toBe('worker-table--down');
    expect(componentRef.getStateColor('READY')).toBe('worker-table--ready');
    expect(componentRef.getStateColor('Ready')).toBe('worker-table--ready');
    expect(componentRef.getStateColor('ReAdy')).toBe('worker-table--ready');
    expect(componentRef.getStateColor('ReAdY')).toBe('worker-table--ready');
  });
